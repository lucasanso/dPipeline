...
# Definindo argumentos para a dag 
default_args = {
    'owner': 'airflow',
...
# Decorador importado do próprio Airflow
@dag(
    dag_id='postgres_to_snowflake',
    default_args=default_args,
    description='Load data incrementally from Postgres to Snowflake',
    schedule=timedelta(days=1),
    catchup=False               # 'catchup' caso estivesse em true iria executar de uma vez só todas as vezes que não foi executada 
)
...
def postgres_to_snowflake_etl():
    ...
    for table_name in table_names:
        @task(task_id=f'get_max_id_{table_name}') # Decorador 'task' importado do próprio Airflow
        ...
        @task(task_id=f'load_data_{table_name}') # Decorador 'task' importado do próprio Airflow
